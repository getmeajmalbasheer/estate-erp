...
st.set_page_config(
    page_title="തോട്ടം പ്രൊഫഷണൽ മാനേജർ", layout="wide", page_icon="🌱"
)
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        st.markdown(
            "<h1 style='text-align: center; font-size: 2rem;'>🌱 തോട്ടം"
            " മാനേജർ</h1>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='text-align: center; color: #718096; margin-bottom:"
            " 30px;'>സുരക്ഷിതമായ എസ്റ്റേറ്റ് ഡിജിറ്റൽ സൊല്യൂഷൻ</p>",
            unsafe_allow_html=True,
        )
        ...
        with tab1:
            with st.form("login_form"):
                ...
                submit = st.form_submit_button(
                    "ലോഗിൻ ചെയ്യുക", use_container_width=True
                )
                ...
                if submit:
                    ...
                    if user_record and check_password(
                        password, user_record["password"]
                    ):
                        st.session_state.logged_in = True
                        st.session_state.username = username
                        st.session_state.role = user_record.get(
                            "role", "worker"
                        )
                        st.rerun()
                    else:
        ...
        with tab2:
            with st.form("signup_form"):
                ...
                confirm_pass = st.text_input(
                    "പാസ്‌വേഡ് വീണ്ടും നൽകുക", type="password"
                )
                signup_submit = st.form_submit_button(
                    "അക്കൗണ്ട് ക്രിയേറ്റ് ചെയ്യുക", use_container_width=True
                )
                    ...
                    if new_user.strip() and new_pass.strip():
                        if new_pass == confirm_pass:
                            ...
                            if new_user.strip() in existing_usernames:
                                ...
                                st.success(
                                    "അക്കൗണ്ട് വിജയകരമായി ക്രിയേറ്റ് ചെയ്തു!"
                                    " ലോഗിൻ ചെയ്യാം."
                                )
                        else:
    ...
    st.sidebar.markdown(
        f"### 🌿 **{curr_user.upper()}** ({curr_role.capitalize()})"
    )
    st.sidebar.markdown("---")
    ...
    if f"labor_data_{curr_user}" not in st.session_state:
        st.session_state[f"labor_data_{curr_user}"] = load_data(LABOR_FILE)
    if f"input_data_{curr_user}" not in st.session_state:
        st.session_state[f"input_data_{curr_user}"] = load_data(INPUT_FILE)
    if f"travel_data_{curr_user}" not in st.session_state:
        st.session_state[f"travel_data_{curr_user}"] = load_data(TRAVEL_FILE)
    if f"harvest_data_{curr_user}" not in st.session_state:
        st.session_state[f"harvest_data_{curr_user}"] = load_data(HARVEST_FILE)
    if f"sales_data_{curr_user}" not in st.session_state:
        st.session_state[f"sales_data_{curr_user}"] = load_data(SALES_FILE)
    if f"worker_data_{curr_user}" not in st.session_state:
        st.session_state[f"worker_data_{curr_user}"] = load_data(WORKER_FILE)
    if f"rainfall_data_{curr_user}" not in st.session_state:
        st.session_state[f"rainfall_data_{curr_user}"] = load_data(RAINFALL_FILE)

    loaded_works = load_data(WORK_FILE)
    if f"work_data_{curr_user}" not in st.session_state:
        st.session_state[f"work_data_{curr_user}"] = (
            loaded_works if loaded_works else default_works
        )

    plots_list = [
        "തറവാട് പറമ്പ് (1 ഏക്കർ)",
        "പുഷ്പക്കണ്ടം (2 ഏക്കർ)",
    ...
    available_menu = ADMIN_MENU if curr_role == "admin" else WORKER_MENU
    menu = st.sidebar.selectbox("നാവിഗേഷൻ മെനു", available_menu)

    # --- 1. DASHBOARD & ANALYTICS (ADMIN ONLY) ---
    if menu == "📊 ഡാഷ്‌ബോർഡ് & അനലിറ്റിക്സ്":
        if curr_role != "admin":
            st.error("⛔ ഈ പേജ് കാണാൻ അഡ്മിൻ അനുമതി ആവശ്യമാണ്!")
            st.stop()

        st.subheader(
            f"📊 {curr_user.upper()} - തോട്ടം വരവു-ചെലവ് സംഗ്രഹവും Outturn %"
            " അനലിറ്റിക്സും"
        )
        ...
        total_labor_and_advance = (
            sum(item.get("തുക/കൂലി", 0) for item in l_data) if l_data else 0
        )
        total_inputs = sum(item.get("വില", 0) for item in i_data) if i_data else 0
        total_travel = (
            sum(item.get("ചെലവ്", 0) for item in t_data) if t_data else 0
        )
        total_sales = (
            sum(item.get("ആകെ തുക", 0) for item in s_data) if s_data else 0
        )
        ...
        if h_data:
            ...
            if (
                not df_h.empty
                and "വിള" in df_h.columns
                and "പച്ച തൂക്കം (kg)" in df_h.columns
                and "ഉണക്ക തൂക്കം (kg)" in df_h.columns
            ):
                cardamom_df = df_h[
                    (df_h["വിള"] == "ഏലം") & (df_h["പച്ച തൂക്കം (kg)"] > 0)
                ]

                if not cardamom_df.empty:
                    df_1ac = cardamom_df[
                        cardamom_df["പ്ലോട്ട്"] == "തറവാട് പറമ്പ് (1 ഏക്കർ)"
                    ]
                    total_green_1ac = df_1ac["പച്ച തൂക്കം (kg)"].sum()
                    total_dry_1ac = df_1ac["ഉണക്ക തൂക്കം (kg)"].sum()
                    if total_green_1ac > 0:
                        outturn_1ac = round(
                            (total_dry_1ac / total_green_1ac) * 100, 2
                        )

                    df_2ac = cardamom_df[
                        cardamom_df["പ്ലോട്ട്"] == "പുഷ്പക്കണ്ടം (2 ഏക്കർ)"
                    ]
                    total_green_2ac = df_2ac["പച്ച തൂക്കം (kg)"].sum()
                    total_dry_2ac = df_2ac["ഉണക്ക തൂക്കം (kg)"].sum()
                    if total_green_2ac > 0:
                        outturn_2ac = round(
                            (total_dry_2ac / total_green_2ac) * 100, 2
                        )
        ...
        with c1:
            st.markdown(
                '<div class="saas-card"><p class="card-title">ആകെ'
                f' ചെലവ്</p><p class="card-value">₹ {total_expense:,.2f}</p><p'
                ' class="card-delta-neg">കൂലി + അഡ്വാൻസ് + വളം +'
                " യാത്ര</p></div>",
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                '<div class="saas-card"><p class="card-title">ആകെ'
                f' വരുമാനം</p><p class="card-value">₹ {total_sales:,.2f}</p><p'
                ' class="card-delta-pos">വിൽപ്പന വഴി ലഭിച്ചത്</p></div>',
                unsafe_allow_html=True,
            )
        with c3:
            profit_class = (
                "card-delta-pos" if net_profit >= 0 else "card-delta-neg"
            )
            profit_text = "ലാഭം" if net_profit >= 0 else "നഷ്ടം"
            st.markdown(
                '<div class="saas-card"><p class="card-title">അന്തിമ'
                f' ഫലം</p><p class="card-value">₹ {net_profit:,.2f}</p><p'
                f' class="{profit_class}">{profit_text}</p></div>',
                unsafe_allow_html=True,
            )
        with c4:
            st.markdown(
                '<div class="saas-card"><p class="card-title">യാത്ര &'
                ' പെട്രോൾ</p><p class="card-value">₹'
                f' {total_travel:,.2f}</p><p class="card-delta-neg">മൊത്തം'
                " യാത്രാച്ചെലവ്</p></div>",
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            "##### 🌿 പ്ലോട്ട് തിരിച്ചുള്ള ഏലക്ക ഉണക്ക് ശതമാനം (Avg Outturn"
            " %)"
        )
        ...
        with col_o1:
            color_1ac = (
                "card-delta-pos" if outturn_1ac >= 18 else "card-delta-neg"
            )
            st.markdown(
                f"""
        ...
        with col_o2:
            color_2ac = (
                "card-delta-pos" if outturn_2ac >= 18 else "card-delta-neg"
            )
            st.markdown(
                f"""
        ...
        with pd.ExcelWriter(output_master, engine="openpyxl") as writer:
            if l_data:
                pd.DataFrame(l_data).to_excel(
                    writer, index=False, sheet_name="Labor_Advance"
                )
                has_data = True
            if i_data:
                pd.DataFrame(i_data).to_excel(
                    writer, index=False, sheet_name="Inputs"
                )
                has_data = True
            if t_data:
                pd.DataFrame(t_data).to_excel(
                    writer, index=False, sheet_name="Travel"
                )
                has_data = True
            if s_data:
                pd.DataFrame(s_data).to_excel(
                    writer, index=False, sheet_name="Sales"
                )
                has_data = True
            if h_data:
                pd.DataFrame(h_data).to_excel(
                    writer, index=False, sheet_name="Harvest_Outturn"
                )
                has_data = True
            if r_data:
                pd.DataFrame(r_data).to_excel(
                    writer, index=False, sheet_name="Rainfall_Log"
                )
                has_data = True

            if not has_data:
                pd.DataFrame([{"സന്ദേശം": "ഡാറ്റയൊന്നും ലഭ്യമല്ല"}]).to_excel(
    ...
    # --- 2. WEATHER & MARKET PRICES ---
        ...
        st.subheader(
            "🌤️ തത്സമയ ഹൈറേഞ്ച് കാലാവസ്ഥ & വിപണി വില (Live Update)"
        )
        ...
        def fetch_weather(city_name):
            ...
            url = (
                ...
                f"?q={city_name}&appid={api_key}&units=metric&lang=ml"
            )
            try:
        ...
        with col_w1:
            ...
            if temp_h:
                ...
                st.info(
                    "🌡️ **അവസ്ഥ:** ഭാഗികമായി മേഘാവൃതം (24°C)\n💡"
                    " **നിർദ്ദേശം:** മഴ സാധ്യത കുറവായതിനാൽ ഇന്ന്"
                    " മരുന്നടിക്കാം."
                )
        ...
        with col_w2:
            ...
            if temp_h:
                st.info(
                    f"🌡️ **താപനില:** {temp_h - 1}°C\n💧 **ഈർപ്പം:**"
                    f" {hum_h + 2}%\n☁️ **അവസ്ഥ:** {desc_h}\n\n💡"
                    " **നിർദ്ദേശം:** വളമിടാൻ അനുകൂല സമയം."
                )
            else:
                st.info(
                    "🌡️ **അവസ്ഥ:** ഈർപ്പമുള്ള കാലാവസ്ഥ (22°C)\n💡"
                    " **നിർദ്ദേശം:** തോട്ടത്തിൽ ആവശ്യത്തിന് ഈർപ്പമുണ്ട്."
                )

        st.write("---")
        st.markdown(
            "### 📈 ഇന്നത്തെ സുഗന്ധവ്യഞ്ജന വിപണി വില (Live Market Rates)"
        )
        ...
        market_data = [
            {
                "വിള": "ഏലം (Small Cardamom - Avg)",
                "ഇന്നത്തെ വില (കിലോയ്ക്ക്)": "₹ 3,150 - ₹ 3,350",
                "ട്രെൻഡ്": "📈 കയറുന്നു",
            },
            {
                "വിള": "ഏലം (Top Quality 8mm+)",
                "ഇന്നത്തെ വില (കിലോയ്ക്ക്)": "₹ 4,100 - ₹ 4,400",
                "ട്രെൻഡ്": "🔥 ഉയർന്ന വില",
            },
            {
                "വിള": "കുരുമുളക് (Garbled)",
                "ഇന്നത്തെ വില (കിലോയ്ക്ക്)": "₹ 620 - ₹ 660",
                "ട്രെൻഡ്": "⚖️ സ്ഥിരം",
            },
            {
                "വിള": "ജാതിക്ക (വിത്തോടു കൂടി)",
                "ഇന്നത്തെ വില (കിലോയ്ക്ക്)": "₹ 260 - ₹ 300",
                "ട്രെൻഡ്": "📈 കയറുന്നു",
            },
            {
                "വിള": "ജാതിപത്രി (Nutmeg Mace)",
                "ഇന്നത്തെ വില (കിലോയ്ക്ക്)": "₹ 1,100 - ₹ 1,350",
                "ട്രെൻഡ്": "🔥 ഉയർന്ന വില",
            },
            {
                "വിള": "ഗ്രാമ്പൂ (Cloves)",
                "ഇന്നത്തെ വില (കിലോയ്ക്ക്)": "₹ 860 - ₹ 910",
                "ട്രെൻഡ്": "⚖️ സ്ഥിരം",
            },
        ]
    ...
    # --- 3. RAINFALL LOG ---
        ...
        st.subheader(
            "🌧️ തോട്ടത്തിലെ ദിനചര്യ മഴ രേഖപ്പെടുത്താൻ (Rain Gauge Log)"
        )
        ...
        with st.form("rain_form", clear_on_submit=True):
            ...
            r_mm = col1.number_input(
                "മഴയുടെ അളവ് (mm - മില്ലീമീറ്ററിൽ)",
                min_value=0.0,
                step=1.0,
                help="റെയിൻ ഗേജിൽ ലഭിച്ച അളവ് നൽക്കുക",
            )
            r_notes = col2.text_input(
                "കുറിപ്പുകൾ (ഉദാ: കനത്ത കാറ്റും മഴയും / ചാറ്റൽ മഴ)"
            )

            if r_mm > 50:
                st.error(
                    "⚠️ **കനത്ത മഴ (Heavy Rain > 50mm):** ഇന്നു തോട്ടത്തിൽ"
                    " മരുന്നടിയോ വളമിടലോ പൂർണ്ണമായും ഒഴിവാക്കുക. ചാലുകളിൽ"
                    " വെള്ളക്കെട്ട് ഉണ്ടാകാതെ നീരൊഴുക്ക് ഉറപ്പാക്കുക."
                )
            elif 15 <= r_mm <= 50:
                st.warning(
                    "🌧️ **മിതമായ മഴ (15mm - 50mm):** മരുന്നടിച്ചാൽ കഴുകിപ്പോകാൻ"
                    " സാധ്യതയുണ്ട്. കവത്തെടുപ്പ് പണികൾ ചെയ്യാം."
                )
            elif 0 < r_mm < 15:
                st.success(
                    "🌦️ **ലഘുവായ മഴ (< 15mm):** തോട്ടത്തിൽ വളമിടാൻ"
                    " അനുയോജ്യമായ നല്ല ഈർപ്പമുണ്ട്."
                )

            if st.form_submit_button(
                "മഴയുടെ അളവ് സേവ് ചെയ്യുക", use_container_width=True
            ):
                ...
                save_data(
                    st.session_state[f"rainfall_data_{curr_user}"],
                    RAINFALL_FILE,
                )
        ...
        if st.session_state[f"rainfall_data_{curr_user}"]:
            ...
            with col_r1:
                if st.button(
                    "മഴ കണക്കുകൾ അപ്ഡേറ്റ് ചെയ്യുക", use_container_width=True
                ):
                    st.session_state[f"rainfall_data_{curr_user}"] = (
                        edited_rain.to_dict("records")
                    )
                    save_data(
                        st.session_state[f"rainfall_data_{curr_user}"],
                        RAINFALL_FILE,
    ...
    # --- 4. LABOR & WAGES ---
        ...
        st.subheader(
            "👷 തൊഴിലാളി കൂലിയും അഡ്വാൻസും രേഖപ്പെടുത്താൻ (Wages & Advance"
            " Log)"
        )
        worker_list = (
            [
                w["തൊഴിലാളി"]
                for w in st.session_state[f"worker_data_{curr_user}"]
            ]
            if st.session_state[f"worker_data_{curr_user}"]
            else []
        )
        work_list = [
            w["പണി"] for w in st.session_state[f"work_data_{curr_user}"]
        ]
        ...
        with st.form("labor_form", clear_on_submit=True):
            ...
            entry_type = col1.selectbox(
                "ഇനം തിരഞ്ഞെടുക്കുക",
                ["കൂലി (Labor Wages)", "അഡ്വാൻസ് (Advance Payment)"],
            )
            worker = col2.selectbox(
                "തൊഴിലാളി",
                worker_list if worker_list else ["തൊഴിലാളികൾ ഇല്ല"],
            )
            work = (
                "അഡ്വാൻസ് പണം"
                if entry_type == "അഡ്വാൻസ് (Advance Payment)"
                else col1.selectbox("പണിയുടെ പേര്", work_list)
            )
            amount = col2.number_input(
                "ആകെ തുക (₹)", min_value=0.0, step=50.0
            )

            col_p1, col_p2 = st.columns(2)
            payment_status = col_p1.selectbox(
                "പേയ്മെന്റ് സ്റ്റാറ്റസ് (Status)",
                ["Paid (നൽകി)", "Pending (നൽകാനുണ്ട്)"],
            )
            payment_mode = col_p2.selectbox(
                "പേയ്മെന്റ് രീതി", ["Cash", "GPay", "Bank Transfer", "Pending / None"]
            )
            ...
            if st.form_submit_button("സേവ് ചെയ്യുക", use_container_width=True):
                if not worker_list:
                    st.error(
                        "തൊഴിലാളി ഇല്ലാതെ സേവ് ചെയ്യാനാകില്ല! 'മാസ്റ്റർ"
                        " ക്രമീകരണങ്ങൾ' ടാബിൽ തൊഴിലാളിയെ ചേർക്കുക."
                    )
                elif amount <= 0:
                    ...
                    save_data(
                        st.session_state[f"labor_data_{curr_user}"], LABOR_FILE
                    )
        ...
        if st.session_state[f"labor_data_{curr_user}"]:
            ...
            df_labor = pd.DataFrame(
                st.session_state[f"labor_data_{curr_user}"]
            )

            if "സ്റ്റാറ്റസ്" in df_labor.columns:
                pending_df = df_labor[
                    df_labor["സ്റ്റാറ്റസ്"] == "Pending (നൽകാനുണ്ട്)"
                ]
                total_pending = (
                    pending_df["തുക/കൂലി"].sum() if not pending_df.empty else 0.0
                )
                if total_pending > 0:
                    st.warning(
                        "⚠️ **ഇതുവരെ തൊഴിലാളികൾക്ക് നൽകാൻ ബാക്കിയുള്ള തുക (Total"
                        f" Pending): ₹ {total_pending:,.2f}**"
                    )
                else:
                    st.success(
                        "✅ നൽകാൻ പെൻഡിംഗ് തുകകളൊന്നും നിലവിലില്ല (All Wages"
                        " Paid)!"
                    )
    ...
    # --- 5. FERTILIZER & DOSAGE ---
        ...
        with st.form("input_form", clear_on_submit=True):
            ...
            dosage = col2.text_input(
                "ഡോസേജ് / അളവ് (ഉദാ: 20 ഗ്രാം / 1 ലിറ്റർ)"
            )
            qty = col1.text_input("വാങ്ങിയ അളവ് (ഉദാ: 2 ചാക്ക് / 5 ലിറ്റർ)")
            cost = col2.number_input("ആകെ വില (₹)", min_value=0.0)

            if st.form_submit_button(
                "വളം/മരുന്ന് സേവ് ചെയ്യുക", use_container_width=True
            ):
                if not item.strip():
                    ...
                    save_data(
                        st.session_state[f"input_data_{curr_user}"], INPUT_FILE
                    )
        ...
        if st.session_state[f"input_data_{curr_user}"]:
            ...
            with col_d1:
                if st.button("അപ്ഡേറ്റ് ചെയ്യുക", use_container_width=True):
                    st.session_state[f"input_data_{curr_user}"] = (
                        edited_input.to_dict("records")
                    )
                    save_data(
                        st.session_state[f"input_data_{curr_user}"], INPUT_FILE
                    )
            ...
            with col_d2:
                ...
                st.download_button(
                    label="📥 എക്സൽ ആയി ഡൗൺലോഡ്",
                    data=excel_data,
                    file_name="inputs_report.xlsx",
                    mime=(
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    ),
                    use_container_width=True,
    ...
    # --- 6. TRAVEL & FUEL EXPENSE (ADMIN ONLY) ---
        ...
        with st.form("travel_form", clear_on_submit=True):
            ...
            travel_cost = col2.number_input(
                "ചെലവായ തുക (₹)", min_value=0.0, step=50.0
            )

            if st.form_submit_button(
                "യാത്രാചെലവ് സേവ് ചെയ്യുക", use_container_width=True
            ):
                if travel_cost <= 0:
                    ...
                    save_data(
                        st.session_state[f"travel_data_{curr_user}"],
                        TRAVEL_FILE,
                    )
    ...
    # --- 7. HARVEST & OUTTURN ---
        ...
        with st.form("harvest_form", clear_on_submit=True):
            ...
            if crop == "ഏലം":
                ...
                green_wt = col_g.number_input(
                    "പച്ച ഏലക്ക തൂക്കം (kg)", min_value=0.0, step=0.5
                )
                dry_wt = col_d.number_input(
                    "ഉണക്ക ഏലക്ക തൂക്കം (kg)", min_value=0.0, step=0.5
                )

                if green_wt > 0 and dry_wt > 0:
                    outturn_pct = round((dry_wt / green_wt) * 100, 2)
                    st.info(
                        "💡 **കണക്കാക്കിയ Outturn (ഉണക്ക് ശതമാനം):"
                        f" {outturn_pct}%**"
                    )

                    if outturn_pct < 18:
                        st.warning(
                            "⚠️ ശ്രദ്ധിക്കുക: ഉണക്ക് ശതമാനം"
                            " സാധാരണയേക്കാൾ കുറവാണ് (Standard Outturn: 18% -"
                            " 22%)."
                        )
                    elif outturn_pct >= 18:
                        st.success(
                            "✅ മികച്ച ഉണക്ക് അനുപാതം (Good Recovery Rate)!"
                        )
            else:
                dry_wt = col2.number_input(
                    "തൂക്കം (kg)", min_value=0.0, step=0.5
                )

            notes = st.text_input("കുറിപ്പുകൾ / ഡ്രയർ ഫീ (ഓപ്ഷണൽ)")

            if st.form_submit_button(
                "വിളവെടുപ്പ് വിവരങ്ങൾ സേവ് ചെയ്യുക", use_container_width=True
            ):
                if crop == "ഏലം" and green_wt <= 0:
                    ...
                    save_data(
                        st.session_state[f"harvest_data_{curr_user}"],
                        HARVEST_FILE,
                    )
        ...
        if st.session_state[f"harvest_data_{curr_user}"]:
            st.write("---")
            st.subheader(
                "📋 കഴിഞ്ഞ വിളവെടുപ്പുകളുടെ റെക്കോർഡുകൾ & Outturn %"
            )
            df_harvest = pd.DataFrame(
                st.session_state[f"harvest_data_{curr_user}"]
            )
            edited_harvest = st.data_editor(
                df_harvest,
                num_rows="dynamic",
                key="harvest_editor",
                use_container_width=True,
            ...
            with col_d1:
                if st.button("അപ്ഡേറ്റ് ചെയ്യുക", use_container_width=True):
                    st.session_state[f"harvest_data_{curr_user}"] = (
                        edited_harvest.to_dict("records")
                    )
                    save_data(
                        st.session_state[f"harvest_data_{curr_user}"],
                        HARVEST_FILE,
    ...
    # --- 8. SALES & REVENUE (ADMIN ONLY) ---
        ...
        with st.form("sales_form", clear_on_submit=True):
            ...
            date = col1.date_input("തീയതി")
            crop = col2.selectbox(
                "വിളയുടെ പേര്", ["ഏലം", "കുരുമുളക്", "ജാതി", "ഗ്രാമ്പൂ"]
            )
            qty = col1.number_input("വിറ്റ അളവ് (കിലോ)", min_value=0.0)
            total_price = col2.number_input(
                "ലഭിച്ച ആകെ തുക (₹)", min_value=0.0
            )
            buyer = st.text_input("വാങ്ങിയ ആളിന്റെ / കടയുടെ പേര്")

            if st.form_submit_button(
                "വരുമാനം സേവ് ചെയ്യുക", use_container_width=True
            ):
                if total_price <= 0:
                    st.warning("തുക നൽകുക.")
                else:
                    ...
                    save_data(
                        st.session_state[f"sales_data_{curr_user}"], SALES_FILE
                    )
                    st.success("സേവ് ചെയ്തു!")
        ...
        if st.session_state[f"sales_data_{curr_user}"]:
            ...
            with col_d1:
                if st.button("അപ്ഡേറ്റ് ചെയ്യുക", use_container_width=True):
                    st.session_state[f"sales_data_{curr_user}"] = (
                        edited_sales.to_dict("records")
                    )
                    save_data(
                        st.session_state[f"sales_data_{curr_user}"], SALES_FILE
                    )
    ...
    # --- 9. MASTER SETTINGS (ADMIN ONLY) ---
        ...
        with col_w1:
            ...
            with st.form("worker_add_form", clear_on_submit=True):
                ...
                if st.form_submit_button(
                    "തൊഴിലാളിയെ സേവ്", use_container_width=True
                ):
                    if w_name.strip():
                        st.session_state[f"worker_data_{curr_user}"].append({
                            "തൊഴിലാളി": w_name.strip(),
                            "ഫോൺ": w_phone,
                        })
                        save_data(
                            st.session_state[f"worker_data_{curr_user}"],
                            WORKER_FILE,
                        )
                        st.success("സേവ് ചെയ്തു!")
            ...
            if st.session_state[f"worker_data_{curr_user}"]:
                ...
                if st.button(
                    "തൊഴിലാളി ലിസ്റ്റ് അപ്ഡേറ്റ്", use_container_width=True
                ):
                    st.session_state[f"worker_data_{curr_user}"] = (
                        edited_w.to_dict("records")
                    )
                    save_data(
                        st.session_state[f"worker_data_{curr_user}"],
                        WORKER_FILE,
        ...
        with col_w2:
            ...
            with st.form("work_add_form", clear_on_submit=True):
                ...
                if st.form_submit_button(
                    "പണി സേവ്", use_container_width=True
                ):
                    if wk_name.strip():
                        existing_w = [
                            w["പണി"]
                            for w in st.session_state[f"work_data_{curr_user}"]
                        ]
                        if wk_name.strip() not in existing_w:
                            st.session_state[
                                f"work_data_{curr_user}"
                            ].append({"പണി": wk_name.strip()})
                            save_data(
                                st.session_state[f"work_data_{curr_user}"],
                                WORK_FILE,
                            )
            ...
            if st.session_state[f"work_data_{curr_user}"]:
                ...
                if st.button(
                    "പണികളുടെ ലിസ്റ്റ് അപ്ഡേറ്റ്", use_container_width=True
                ):
                    st.session_state[f"work_data_{curr_user}"] = (
                        edited_wk.to_dict("records")
                    )
                    save_data(
                        st.session_state[f"work_data_{curr_user}"], WORK_FILE
                    )
                    st.success("അപ്ഡേറ്റ് ചെയ്തു!")
                    st.rerun()

        # --- ADMIN-ONLY USER & ROLE MANAGEMENT PANEL ---
        st.write("---")
        st.markdown("### 👥 സിസ്റ്റം യൂസർ & റോൾ മാനേജ്‌മെന്റ് (Admin Panel)")
        st.caption("യൂസറുകളുടെ അനുമതികളും (Admin / Worker) പാസ്‌വേഡും ഇവിടെ മാറ്റാം.")
        ...
        if not df_users.empty:
            edited_df = st.data_editor(
                ...
                column_config={
                    "username": st.column_config.TextColumn(
                        "യൂസർ നെയിം", disabled=True
                    ),
                    "role": st.column_config.SelectboxColumn(
                        "റോൾ (അനുമതി)",
                        options=["admin", "worker"],
                        required=True,
            ...
            if st.button("റോൾ മാറ്റങ്ങൾ സേവ് ചെയ്യുക", use_container_width=True):
                ...
                for record in updated_records:
                    record["password"] = pass_map.get(record["username"], "")

                st.session_state.users_data = updated_records
                save_data(st.session_state.users_data, USERS_FILE)

                st.success("യൂസർ റോളുകൾ വിജയകരമായി അപ്ഡേറ്റ് ചെയ്തു!")
                st.rerun()
            ...
            with st.form("admin_password_reset_form", clear_on_submit=True):
                target_user = st.selectbox("യൂസർ തിരഞ്ഞെടുക്കുക", all_usernames)
                new_reset_pass = st.text_input("പുതിയ പാസ്‌വേഡ്", type="password")
                confirm_reset_pass = st.text_input(
                    "പുതിയ പാസ്‌വേഡ് വീണ്ടും നൽകുക", type="password"
                )

                if st.form_submit_button(
                    "പാസ്‌വേഡ് മാറ്റുക", use_container_width=True
                ):
                    if not new_reset_pass.strip():
                        st.warning("പുതിയ പാസ്‌വേഡ് നൽകുക.")
                    elif new_reset_pass != confirm_reset_pass:
                        st.error("നൽകിയ പാസ്‌വേഡുകൾ ഒരുപോലെയല്ല!")
                    else:
                        new_hashed = hash_password(new_reset_pass.strip())
                        ...
                        save_data(st.session_state.users_data, USERS_FILE)
                        st.success(
                            f"**{target_user}** -ന്റെ പാസ്‌വേഡ് വിജയകരമായി"
                            " മാറ്റിയ നൽകി!"
                        )
